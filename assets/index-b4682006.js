(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))i(t);new MutationObserver(t=>{for(const o of t)if(o.type==="childList")for(const n of o.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&i(n)}).observe(document,{childList:!0,subtree:!0});function r(t){const o={};return t.integrity&&(o.integrity=t.integrity),t.referrerPolicy&&(o.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?o.credentials="include":t.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(t){if(t.ep)return;t.ep=!0;const o=r(t);fetch(t.href,o)}})();const u="/assets/smart-f7e1cb05.webp";document.querySelector("#app").innerHTML=`
<div class="container">
        <div class="product">
            <div class="product-image">
                <img src="${u}" alt="Product Image">
            </div>
...
`;const p="http://localhost:8000",m={backgroundColor:"#007bff",color:"#fff",marginTop:"16px"},f=739;document.addEventListener("DOMContentLoaded",function(){const c=document.querySelector(".product-details"),e=document.createElement("button");e.type="button",e.className="tc-btn",e.textContent="Share & Earn",Object.assign(e.style,m),c.appendChild(e),e.addEventListener("click",function(){const r=document.createElement("div");r.className="tc-promoter-modal",r.innerHTML=`
			...
			`;const d=await s.createPromoter(o.email.value);t.innerHTML=`
			<label for="tracking_code">Your Tracking Link</label>
			<div class="form-group__row">
				<input value="${d}" disabled type="input" id="tc_tracking_code" name="tracking_code">
				<button id="tc_copy_tracking_code" type="button" class="tc-btn">Copy</button>
			</div>
			`;const a=t.querySelector("#tc_copy_tracking_code");a.addEventListener("click",function(){const l=t.querySelector("#tc_tracking_code");s.copyToClipboard(l.value),a.textContent="Copied",a.style.backgroundColor="#4CAF50",setTimeout(()=>{a.textContent="Copy",a.style.backgroundColor="#007bff"},2e3)})})})});const s={copyToClipboard:function(c){const e=document.createElement("textarea");e.value=c,e.style.position="fixed",e.style.opacity=0,document.body.appendChild(e),e.select(),navigator.clipboard.writeText(e.value).then(()=>{console.info("Text copied to clipboard: ",e.value)}).catch(r=>{console.error("Failed to copy text: ",r)}),document.body.removeChild(e)},createPromoter:async function(c){try{const e=await fetch(`${p}/api/pl/pub/promoter/add`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:c,offerId:f})}),{trackingLink:r}=await e.json();return r}catch(e){return console.error(e),e}}};
