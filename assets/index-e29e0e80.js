...
`;const y="https://staging.targetcircle.com",b={backgroundColor:"#007bff",color:"#fff",marginTop:"16px"},v=739;document.addEventListener("DOMContentLoaded",function(){const c=document.querySelector(".product-details"),e=document.createElement("button");e.type="button",e.className="tc-btn",e.textContent="Share & Earn",Object.assign(e.style,b),c.appendChild(e),e.addEventListener("click",function(){const r=document.createElement("div");r.className="tc-promoter-modal",r.innerHTML=`
			...
			`,document.body.appendChild(r),r.querySelector(".tc-promoter-modal__overlay").addEventListener("click",function(){document.body.removeChild(r)});const t=r.querySelector("#tc_tracking_code"),o=r.querySelector(".tc-promoter-modal__form");o.addEventListener("submit",async function(n){var d;n.preventDefault(),t.innerHTML=`
			<div class="form-group__row">
				<button class="tc-btn">Loading...</button>
			</div>
			`;const s=(d=document.querySelector("link[rel='canonical']"))==null?void 0:d.href,{trackingLink:u,success:p}=await l.createPromoter(o.email.value);t.innerHTML=`
			<label for="tracking_code">Your Tracking Link</label>
			<div class="form-group__row">
				<input value="${u}${p&&s?"?deeplink="+s:""}" disabled type="input" id="tc_tracking_code" name="tracking_code">
				<button id="tc_copy_tracking_code" type="button" class="tc-btn">Copy</button>
			</div>
			`;const i=t.querySelector("#tc_copy_tracking_code");i.addEventListener("click",function(){const m=t.querySelector("#tc_tracking_code");l.copyToClipboard(m.value),i.textContent="Copied",i.style.backgroundColor="#4CAF50",setTimeout(()=>{i.textContent="Copy",i.style.backgroundColor="#007bff"},2e3)})})})});const l={copyToClipboard:function(c){const e=document.createElement("textarea");e.value=c,e.style.position="fixed",e.style.opacity=0,document.body.appendChild(e),e.select(),navigator.clipboard.writeText(e.value).then(()=>{console.info("Text copied to clipboard: ",e.value)}).catch(r=>{console.error("Failed to copy text: ",r)}),document.body.removeChild(e)},createPromoter:async function(c){try{const e=await fetch(`${y}/api/pl/pub/promoter/add`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:c,offerId:v})}),{trackingLink:r}=await e.json();return{trackingLink:r,success:!0}}catch(e){return console.error(e),{trackingLink:"Could not create tracking link",success:!1}}}};
